...
namespace SDF
{
	...
	void Octree::OutputTreeSlice(const char* fName_, const f64 c_)
	{
		// Obtain mem and set size
		const usize width = 2048;
		const usize height = 2048;
		...
		for (usize i = 0; i < width; ++i)
		{
			for (usize j = 0; j < height; ++j)
			{
				...
				// Determine degree of poly reached
				usize curNodeIdx = 0;
				usize degree = 0;
				while (!degree)
				{
					const usize firstChildIdx = nodes[curNodeIdx].childIdx;
					for (usize i = 0; i < 8; ++i)
					{
						if (nodes[firstChildIdx + i].aabb.contains(sampleLoc))
						{
							if (nodes[firstChildIdx + i].basis.degree != 0)
							{
								// Leaf
								degree = nodes[firstChildIdx + i].basis.degree;
							}

							curNodeIdx = firstChildIdx + i;
							break;
						}
					}
				}

				// Write bytes. c.f. https://i.pinimg.com/236x/4b/27/7f/4b277f246eedeb8597b996276436fb41--cmyk-color-palette-color-palettes.jpg
				switch (degree)
				{
					case 0:
					{
						imageBytes[3 * (i * height + j)] = 201;
						imageBytes[3 * (i * height + j) + 1] = 201;
						imageBytes[3 * (i * height + j) + 2] = 201;
						break;
					}

					case 1:
					{
						imageBytes[3 * (i * height + j)] = 178;
						imageBytes[3 * (i * height + j) + 1] = 162;
						imageBytes[3 * (i * height + j) + 2] = 150;
						break;
					}

					case 2:
					{
						imageBytes[3 * (i * height + j)] = 103;
						imageBytes[3 * (i * height + j) + 1] = 124;
						imageBytes[3 * (i * height + j) + 2] = 138;
						break;
					}

					case 3:
					{
						imageBytes[3 * (i * height + j)] = 149;
						imageBytes[3 * (i * height + j) + 1] = 212;
						imageBytes[3 * (i * height + j) + 2] = 122;
						break;
					}

					case 4:
					{
						imageBytes[3 * (i * height + j)] = 82;
						imageBytes[3 * (i * height + j) + 1] = 204;
						imageBytes[3 * (i * height + j) + 2] = 206;
						break;
					}
					...
					{
						imageBytes[3 * (i * height + j)] = 0;
						imageBytes[3 * (i * height + j) + 1] = 176;
						imageBytes[3 * (i * height + j) + 2] = 179;
						break;
					}

					case 6:
					{
						imageBytes[3 * (i * height + j)] = 63;
						imageBytes[3 * (i * height + j) + 1] = 100;
						imageBytes[3 * (i * height + j) + 2] = 126;
						break;
					}

					case 7:
					{
						imageBytes[3 * (i * height + j)] = 160;
						imageBytes[3 * (i * height + j) + 1] = 158;
						imageBytes[3 * (i * height + j) + 2] = 214;
						break;
					}

					case 8:
					{
						imageBytes[3 * (i * height + j)] = 75;
						imageBytes[3 * (i * height + j) + 1] = 37;
						imageBytes[3 * (i * height + j) + 2] = 109;
						break;
					}

					case 9:
					{
						imageBytes[3 * (i * height + j)] = 239;
						imageBytes[3 * (i * height + j) + 1] = 62;
						imageBytes[3 * (i * height + j) + 2] = 91;
						break;
					}

					default:
						assert(0);	// TREE_MAX_DEPTH > 9
				}
			}
		}
		...
		for (usize i = 0; i < width; ++i)
		{
			for (usize j = 0; j < height; ++j)
			{
				...
				if (curNodeIdx != prevNodeIdx)
				{
					imageBytes[3 * (i * height + j)] = 0;
					imageBytes[3 * (i * height + j) + 1] = 0;
					imageBytes[3 * (i * height + j) + 2] = 0;

					imageBytes[3 * (j * width + i)] = 0;
					imageBytes[3 * (j * width + i) + 1] = 0;
					imageBytes[3 * (j * width + i) + 2] = 0;
				}
